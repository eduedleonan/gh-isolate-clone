...
# 2. DETECCIÓN DE ENTORNO: ¿Carpeta vacía o con archivos?
# Contamos cuántos archivos hay (excluyendo . y ..)
CANT_ARCHIVOS=$(find . -maxdepth 1 ! -name "." | wc -l)

if [ "$CANT_ARCHIVOS" -eq 0 ]; then
    # --------------------------------------------------------
    # MODO A: CARPETA VACÍA -> MODO DESCARGA / CLONACIÓN
    # --------------------------------------------------------
    echo "----------------------------------------------------"
    ...
    while [ -z "$REPO_NAME" ]; do
        read -p "❌ El nombre no puede estar vacío: " REPO_NAME
    done
    ...
    if ! git ls-remote "$URL_REMOTA" &>/dev/null; then
        echo "❌ ERROR: No se encontró el repositorio '$REPO_NAME' o tu llave no tiene acceso."
        exit 1
    fi
    
    echo "📥 Descargando repositorio en la carpeta actual..."
    # Clonamos el contenido directamente EN ESTA CARPETA (.) en lugar de crear una subcarpeta
    git clone "$URL_REMOTA" .
    
    echo "===================================================="
    echo "🎉 ¡Hecho! Repositorio descargado y listo para trabajar."
    echo "===================================================="

else
    # --------------------------------------------------------
    # MODO B: CARPETA CON CONTENIDO -> MODO CONFIGURACIÓN / PUSH
    # --------------------------------------------------------
    echo "----------------------------------------------------"
    ...
    # Limpieza e Inicialización limpia
    if [ -d ".git" ]; then
